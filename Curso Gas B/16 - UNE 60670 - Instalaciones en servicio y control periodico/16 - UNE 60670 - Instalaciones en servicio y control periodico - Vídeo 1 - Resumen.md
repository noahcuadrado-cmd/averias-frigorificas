...
<p class="eyebrow">Tema 16 · Vídeo 1 de 4 · UNE 60670-11 · Operaciones en instalaciones en servicio</p>
...
> **Nota aclaratoria —** «MOP ≤ 5 bar» es el terreno de juego de Gas B: la baja y media presión de las instalaciones receptoras domésticas y comerciales típicas. Si una instalación va por encima de 5 bar, ya no juega esta norma sino otras. Grábate el número: **5 bar es la frontera de la UNE 60670**.
...
> **Nota aclaratoria — «operar en carga».** La regla general es: **antes de tocar tubería, corta el gas**. La única excepción es que uses **técnicas específicas para trabajar con la instalación con gas dentro** (operar en carga), que requieren medios y formación adecuados. Si no dominas esa técnica, la respuesta siempre es: **cierra primero**.
...
>
> - Tramo **≤ 1 m** (aunque cambies material o trazado) → **REPARACIÓN**. Basta comprobar la estanquidad del tramo reparado (método del 6.1).
> - Tramo **> 1 m** con cambio de material o trazado, **o** ampliar consumo, **o** cambiar un aparato por otro de distintas características → **MODIFICACIÓN**. Exige **prueba de estanquidad completa** según la UNE 60670-8.
>
...
Antes de desmontar el contador se debe **asegurar la continuidad eléctrica entre la entrada y la salida** colocando un **puente antichispas** si no existe aquélla, que debe **retirarse** cuando se haya instalado el nuevo contador.

Una vez sustituido el contador se debe proceder al **precintado del equipo de medida**.

> **Nota aclaratoria — el puente antichispas.** Al cortar la tubería para quitar el contador, puedes interrumpir la continuidad eléctrica que había de un lado a otro. Si por esos tubos circula una pequeña corriente, en el momento de separar podría saltar una **chispa** justo donde hay gas. El **puente antichispas** une eléctricamente entrada y salida **antes** de cortar, para que la corriente pase por el puente y no por el punto de corte. Se pone antes, se quita al final. Regla: **contador nuevo, precintado; contador viejo, puente antes de tocarlo.**
...
Las instalaciones calificadas como **no aptas para uso** se deben **dejar fuera de servicio en el mismo momento** en que se localicen las fugas, **precintando la llave** de la instalación que aísle el tramo afectado.
...
- La **Parte 11** regula las operaciones sobre instalaciones **ya en servicio** (MOP ≤ 5 bar). De toda operación **debe quedar evidencia documental**.
- **Tabla 1:** la **distribuidora** maneja acometida, contador y cese; la **instaladora**, modificaciones, reparaciones, corrección de defectos y anulación de puntos; el **SAT**, solo los **aparatos**. La **llave de acometida** solo la toca la distribuidora o quien ella autorice.
- **Seguridad general:** no fumar, nada de fuegos/focos calientes, no tocar llaves precintadas, cerrar el gas antes de modificar/ampliar (salvo operar en carga) y evitar la mezcla aire-gas inflamable al vaciar.
- **Con indicios de gas:** **no accionar interruptores** (ni apagar luces), no generar chispas ni llamas, ventilar, cerrar la llave, detectores **UNE-EN 61779-1/-4** y **linternas de seguridad**.
- **Reparación vs modificación:** frontera en **1 m**. Reparación (≤ 1 m) → comprobar estanquidad del tramo; modificación (> 1 m, ampliar consumo o aparato distinto) → **prueba de estanquidad UNE 60670-8**.
- **Cambio de contador:** solo personal autorizado por la distribuidora; **puente antichispas** antes de desmontar y **precintado** del equipo tras montar.
- **Estanquidad:** detector portátil (calibración/verificación **≤ 18 meses**), manómetro de esfera clase 1,6 (**35 %-75 %** de escala) o digital/columna de agua, o giro de la métrica del contador (resolución ≥ 1 litro, esperar **5 minutos**). **Fugas: agua jabonosa o detector, nunca llama.**
- Fuga → decidir **apta / pendiente de corrección / no apta**; las no aptas, **fuera de servicio ya** y precintar; en pendiente o no apta, **informar a la distribuidora**.
